...
- (void)setClockView:(WBClockView *)clockView {
    _clockView = clockView;
    [self addSubview:_clockView];
    [self->_statusLabel removeFromSuperview];
    [self->_timeLabel removeFromSuperview];
    [self->_arrowImage removeFromSuperview];
    [self->_activityView removeFromSuperview];
}

- (void)setStateText {
    self.normalStateText = @"每日 00:00 更新内容";
...
- (void)scrollViewContentOffsetDidChange {
    ...
    [self.clockView rotationWithScrollViewContentOffSetY:self.scrollView.contentOffset.y];
    ...
    if (_refreshState != refreshState) {
        ...
        switch (refreshState) {
            case WBRefreshStateNormal:
            {
                ...
                self.clockView.stateText = self.normalStateText;
            ...
            case WBRefreshStatePulling:
            {
                ...
                self.clockView.stateText = self.pullingStateText;
                [UIView animateWithDuration:0.2 animations:^{
            ...
            case WBRefreshStateLoading:
            {
                ...
                self.clockView.stateText = self.loadingStateText;
